...
import { mockHistory, mockSet } from './testFactories'
...
function makeSet_(historyId: string, weight: number, reps: number) {
  return mockSet({
    historyId,
    weight,
    reps,
...
describe('computeVolumeRecords', () => {
  ...
  it('record session = max volume d\'une seule séance', () => {
    ...
    const sets = [
      makeSet_('h1', 100, 10), // 1000
      makeSet_('h2', 50, 10),  // 500
      makeSet_('h2', 50, 10),  // 500 → h2 total = 1000
      makeSet_('h3', 200, 10), // 2000
    ]
  ...
  it('record semaine = max volume sur 7 jours', () => {
    ...
    const histories = [
      makeHistory_('h1', 1),
      makeHistory_('h2', 2),
      makeHistory_('h3', 14), // 2 semaines avant
    ]
    const sets = [
      makeSet_('h1', 100, 10), // 1000
      makeSet_('h2', 100, 10), // 1000 → semaine récente = 2000
      makeSet_('h3', 50, 10),  // 500 → autre semaine = 500
    ]
    const result = computeVolumeRecords(histories, sets)
  ...
  it('isNewRecord = true si volume en cours > record', () => {
    ...
    const histories = [makeHistory_('h1', 1)]
    const sets = [makeSet_('h1', 100, 10)] // 1000
    const result = computeVolumeRecords(histories, sets)
    const sessionRecord = result.records.find(r => r.type === 'session')!
  ...
  it('recentTrend = up si volume récent > volume précédent', () => {
    ...
    const histories = [
      ...
      makeHistory_('h2', 3),
      makeHistory_('h3', 5),
      makeHistory_('h4', 20), // période précédente
    ]
    const sets = [
      makeSet_('h1', 100, 10), // 1000
      makeSet_('h2', 100, 10), // 1000
      makeSet_('h3', 100, 10), // 1000 → recent = 3000
      makeSet_('h4', 50, 10),  // 500 → previous = 500
    ]
    const result = computeVolumeRecords(histories, sets)
